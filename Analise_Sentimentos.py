...
for word in listaStop : #Preparando e guardando as Stopwords.
...
def readTrainingSet(fname):
    ...
    for linha in listaTreino:
        palavrasSeparadas = list(split_on_separators(linha[2:]," ")) #Quebrando afrase em uma lista de palavras.
    ...
    for palavra in listaPalavras:
        if palavra not in notas: #Triagem para não receber inteiros invés de texto
            listaEscores = []
...
def readTestSet(fname):
    ...
    for i in reviews2 :
        palavras_separadas = list(split_on_separators(i[1]," ")) 
        ...
        reviews.append((int(i[0]), comentario))
...
def QuadradoDosErros(reviews,words):
    ...
    comentarios = len(reviews)

    sse = diferencaTotal/comentarios
